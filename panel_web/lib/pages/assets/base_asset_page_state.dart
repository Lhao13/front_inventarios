...
import 'package:panel_web/utils/asset_utils.dart';
import 'package:panel_web/main.dart';
import 'package:panel_web/widgets/multi_select_dialog.dart';
import 'package:panel_web/utils/asset_filter.dart';
import 'package:panel_web/widgets/maintenance_form_dialog.dart';
import 'package:panel_web/widgets/material_list_paginator.dart';
import 'package:panel_web/widgets/asset_data_table.dart';
import 'package:panel_web/pages/assets/dynamic_asset_form.dart';
import 'package:panel_web/pages/asset_detail_page.dart';
...
abstract class BaseAssetPageState<T extends StatefulWidget> extends State<T> {
  ...
  Future<void> _loadAssets({bool showLoading = true}) async {
    ...
    try {
      ...
      // Build activo query with related data
      var activoQuery = supabase.from('activo').select(
        'id, nombre, codigo, numero_serie, categoria_activo, ip, coordenada,'
        'fecha_adquisicion, fecha_entrega, id_tipo_activo, id_condicion_activo,'
        'id_custodio, id_ciudad_activo, id_sede_activo, id_area_activo, id_provedor,'
        'tipo_activo(id, tipo, categoria), condicion_activo(id, condicion),'
        'sede_activo(id, sede), area_activo(id, area), ciudad_activo(id, ciudad),'
        'custodio(id, nombre_completo, email), proveedor(id, nombre),'
        'info_pc(id, id_marca, modelo, procesador, almacenamiento, ram, cargador_codigo, num_puertos, observaciones, marca(id, marca_proveedor)),'
        'info_equipo_comunicacion(id, id_marca, modelo, num_puertos, tipo_extension, observaciones, marca(id, marca_proveedor)),'
        'info_equipo_generico(id, id_marca, modelo, cargador_codigo, num_conexiones, var_impresora_color, var_monitor_tipo_conexion, observaciones, marca(id, marca_proveedor)),'
        'info_software(id, proveedor, fecha_inicio, fecha_fin, observaciones)',
      );

      if (categoryName != null) {
        activoQuery = activoQuery.eq('categoria_activo', categoryName!) as dynamic;
      }

      final activoRes = await activoQuery.order('nombre');
      ...
      final futures = await Future.wait([
        ...
        supabase.from('custodio').select('id, nombre_completo, email').order('nombre_completo'),
      ...
      if (mounted) {
        setState(() {
          final rawActivos = List<Map<String, dynamic>>.from(activoRes);
          // Normalize nested lists
          _allAssets = rawActivos.map((a) {
            final copy = Map<String, dynamic>.from(a);
            // Ensure info sub-lists are always lists
            for (final key in ['info_pc', 'info_equipo_comunicacion', 'info_equipo_generico', 'info_software']) {
              final v = copy[key];
              if (v == null) copy[key] = [];
              else if (v is Map) copy[key] = [v];
            }
            // Normalize foreign keys to int if they come as strings
            for (final fk in ['id_tipo_activo', 'id_condicion_activo', 'id_custodio',
                'id_ciudad_activo', 'id_sede_activo', 'id_area_activo', 'id_provedor']) {
              final val = copy[fk];
              if (val is String) copy[fk] = int.tryParse(val);
            }
            return copy;
          }).toList();

          final tiposAll = List<Map<String, dynamic>>.from(futures[0]);
          _tiposActivo = categoryName != null
              ? tiposAll.where((t) => t['categoria'] == categoryName).toList()
              : tiposAll;
          _condiciones = List<Map<String, dynamic>>.from(futures[1]);
          _sedes = List<Map<String, dynamic>>.from(futures[2]);
          _areas = List<Map<String, dynamic>>.from(futures[3]);
          _ciudades = List<Map<String, dynamic>>.from(futures[4]);
          _custodios = List<Map<String, dynamic>>.from(futures[5]);
      ...
      if (mounted) {
        context.showSnackBar('Error cargando activos: $e', isError: true);
  ...
  Future<void> _deleteAsset(String id) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('¿Deseas eliminar este activo permanente? Esta acción no se puede deshacer.'),
        actions: [
          TextButton(
    ...
    try {
      await Supabase.instance.client.rpc('eliminar_activo', params: {'p_id_activo': id});
                          ...
                          try {
                            ...
                            if (rpcName.isNotEmpty) {
                              ...
                              context.showSnackBar(
                                isUpdate ? 'Activo actualizado correctamente.' : 'Activo creado correctamente.',
                              );
