...
class _AssetManagementPageState extends State<AssetManagementPage> {
  ...
  Future<void> _loadAssets({bool showLoading = true}) async {
    ...
    try {
      ...
      final activoRes = await supabase.from('activo').select(
      ...
      if (mounted) {
        setState(() {
          ...
          _allAssets = List<Map<String, dynamic>>.from(activoRes);
  ...
  Widget _buildTableSection() {
    return AssetDataTable(
      ...
      onEdit: RoleService.currentRole != UserRole.unknown
          ? (asset) async {
          ...
          : null,
      onDelete: (RoleService.currentRole != UserRole.unknown &&
              RoleService.currentRole != UserRole.unknown)
          ? (id) async => _deleteAsset(id)
          : null,
      customActionsBuilder: (asset) {
        final category = asset['categoria_activo']?.toString().toUpperCase();
        if (RoleService.currentRole == UserRole.unknown ||
            RoleService.currentRole == UserRole.unknown ||
            category == 'SOFTWARE') {
          return [];
        }
  ...
  Widget _buildListSection() {
    ...
    if (_filteredAssets.isEmpty) {
      if (_allAssets.isEmpty) {
        return const Center(child: Text('No hay activos en el inventario.'));
      } else {
        return const Center(
