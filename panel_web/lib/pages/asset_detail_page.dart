...
class _AssetDetailPageState extends State<AssetDetailPage> {
  ...
  Future<void> _refreshAsset() async {
    try {
      final assetId = _currentAsset['id']?.toString();
      ...
      final futures = await Future.wait([
        Supabase.instance.client
            ...
            .select(
              ...
              'fecha_adquisicion, fecha_entrega, tipo_activo(id, tipo), condicion_activo(id, condicion),'
              'sede_activo(id, sede), area_activo(id, area), custodio(id, nombre_completo),'
        ...
        Supabase.instance.client
            .from('mantenimiento')
            .select('id, tipo, estado, fecha_programada, fecha_realizada, observacion')
      ...
      if (mounted) {
        setState(() {
          if (futures[0] != null) {
            _currentAsset = Map<String, dynamic>.from(futures[0]! as Map);
          }
          _maintenanceHistory = List<Map<String, dynamic>>.from(futures[1] as List);
        });
      }
    } catch (e) {
  ...
  Future<void> _deleteMaintenance(String id) async {
    ...
    try {
      await Supabase.instance.client.from('mantenimiento').delete().eq('id', id);
      if (mounted) context.showSnackBar('Mantenimiento eliminado.');
      _refreshAsset();
    } catch (e) {
