...
class _GenericAssetsPageState extends State<GenericAssetsPage> {
  ...
  static final List<AssetColumnDef> _genericColumns = [
    ...
    AssetColumnDef(
      label: 'Marca',
      getValue: (a) {
        final i = _info(a, 'info_equipo_generico');
    ...
    AssetColumnDef(
      ...
      getValue: (a) {
        final i = _info(a, 'info_equipo_generico');
        return i?['cargador_codigo']?.toString() ?? 'N/A';
      },
    ),
    AssetColumnDef(
      ...
      getValue: (a) {
        final i = _info(a, 'info_equipo_generico');
        return i?['num_conexiones']?.toString() ?? 'N/A';
      },
    ),
    AssetColumnDef(
      label: 'Observaciones',
      ...
      if (mounted) {
        setState(() {
          ...
          _allAssets = localActivos
              .where((a) => a['categoria_activo'] == 'GENERICO')
              .toList();
          _isLoading = false;
        });
        _applyFilters();
                        ...
                        ({
                          ...
                          try {
                            ...
                            if (isUpdate) {
                              ...
                              if (!mounted) return;
                              context.showSnackBar(
                                'Activo Genérico actualizado localmente.',
                              );
                            } else {
                              ...
                              if (!mounted) return;
                              context.showSnackBar(
                                'Activo Genérico creado localmente.',
                              );
                            }
