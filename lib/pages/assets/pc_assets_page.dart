...
class _PcAssetsPageState extends State<PcAssetsPage> {
  ...
  static final List<AssetColumnDef> _pcColumns = [
    ...
    AssetColumnDef(
      ...
      getValue: (a) {
        ...
        return info?['marca']?['marca_proveedor']?.toString() ?? 'N/A';
      },
    ),
    AssetColumnDef(
      label: 'Modelo',
      getValue: (a) {
        final info = _info(a, 'info_pc');
        return info?['modelo']?.toString() ?? 'N/A';
      },
    ),
    AssetColumnDef(
      label: 'Procesador',
      getValue: (a) {
        final info = _info(a, 'info_pc');
        return info?['procesador']?.toString() ?? 'N/A';
      },
    ),
    AssetColumnDef(
      label: 'RAM',
    ...
    AssetColumnDef(
      ...
      getValue: (a) {
        final info = _info(a, 'info_pc');
        return info?['almacenamiento']?.toString() ?? 'N/A';
      },
    ),
    AssetColumnDef(
      label: 'Cód. Cargador',
      getValue: (a) {
        final info = _info(a, 'info_pc');
        return info?['cargador_codigo']?.toString() ?? 'N/A';
      },
      visibleByDefault: false,
    ),
    AssetColumnDef(
      ...
      getValue: (a) {
        final info = _info(a, 'info_pc');
        return info?['num_puertos']?.toString() ?? 'N/A';
      },
      visibleByDefault: false,
    ),
    AssetColumnDef(
      ...
      getValue: (a) {
        final info = _info(a, 'info_pc');
        return info?['observaciones']?.toString() ?? 'N/A';
      },
  ...
  bool _assetMatches(Map<String, dynamic> asset, {String? ignoreField}) {
    ...
    bool matchesCpu =
        ignoreField == 'procesador' ||
        _selectedCpu.isEmpty ||
        _selectedCpu.contains((info?['procesador'] ?? '').toString());
    bool matchesRam =
        ignoreField == 'ram' ||
        _selectedRams.isEmpty ||
        _selectedRams.contains(info?['ram']?.toString());
    bool matchesStorage =
        ignoreField == 'almacenamiento' ||
        _selectedStorages.isEmpty ||
        _selectedStorages.contains(info?['almacenamiento']?.toString());
    ...
    try {
      ...
      if (mounted)
        context.showSnackBar(
          'Activo eliminado de manera local (Cola activada).',
        );
      _loadAssets();
    } catch (e) {
              ...
              children: [
                Row(
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        isUpdate ? 'Actualizar PC' : 'Agregar Especial - PC',
                        ...
                        ({
                          ...
                          try {
                            ...
                            if (isUpdate) {
                              ...
                              if (!mounted) return;
                              context.showSnackBar(
                                'Activo PC actualizado localmente.',
                              );
                            } else {
  ...
  Widget _buildStringDrawerFilterButton(
    String label,
    ...
    return ListTile(
      ...
      trailing: const Icon(Icons.arrow_drop_down),
      onTap: () async {
        final List<Map<String, dynamic>> itemsList = allItems
            .map((e) => {'id': e})
            .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Equipos PC', textScaler: TextScaler.linear(1)),
        actions: [
          IconButton(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _buildDrawerFilterButton<String>(
                      'Modelos',
                      _selectedModelos,
                      _getUniquePredictiveList('modelo', subKey: 'info_pc'),
                      'valor',
                    ),
                    _buildDrawerFilterButton<String>(
                      'Procesadores',
                      _selectedCpu,
                      _getUniquePredictiveList('procesador', subKey: 'info_pc'),
                      'valor',
                    ),
                    ...
                    _buildStringDrawerFilterButton(
                      'RAM',
                      _selectedRams,
                      _ramOptions,
                    ),
