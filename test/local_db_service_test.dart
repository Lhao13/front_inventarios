...
void main() {
  ...
  group('LocalDbService', () {
    ...
    test('enqueueOperation stores a pending operation and optimistic cache entry', () async {
      final params = {
    ...
    test('updateOperationStatus and removeOperation manage queue entries', () async {
      await LocalDbService.instance.enqueueOperation('table:mantenimiento:insert', {
        'id': 'mtt-1',
        'nombre': 'Mantenimiento test',
      });

      final pendingBefore = await LocalDbService.instance.getPendingOperations();
      expect(pendingBefore, hasLength(1));

      final opId = pendingBefore.first['id'] as String;
      await LocalDbService.instance.updateOperationStatus(opId, 'failed', errorMsg: 'Error de prueba');

      final pendingAfterUpdate = await LocalDbService.instance.getPendingOperations();
      expect(pendingAfterUpdate, hasLength(1));
      expect(pendingAfterUpdate.first['status'], 'failed');
      expect(pendingAfterUpdate.first['error_msg'], contains('Error de prueba'));

      await LocalDbService.instance.removeOperation(opId);
      final pendingAfterRemove = await LocalDbService.instance.getPendingOperations();
